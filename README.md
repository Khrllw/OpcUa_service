...
![OPC UA](https://img.shields.io/badge/OPC%20-UA-orange)
![alt text](https://img.shields.io/badge/license-MIT-lightgrey)

</div> 

**OpcUa Service** — это высокоуровневый RESTful API сервис, построенный на основе **FastAPI**, который предоставляет
простой и стандартизированный интерфейс для взаимодействия с OPC UA серверами. Сервис абстрагирует сложности native OPC
UA протокола, позволяя получать данные телеметрии, метаданные и исторические значения через простые HTTP-запросы.

## 🚀 Возможности

* **REST API**: Полностью RESTful интерфейс для работы с OPC UA сервером.
* **Чтение данных**: Получение текущих значений тегов по их NodeID.
* **Чтение атрибутов**: Получение метаинформации о узлах (атрибуты `NodeClass`, `BrowseName`, `Description` и др.).
* **Просмотр дерева узлов**: Навигация по дереву объектов OPC UA сервера starting from the root.
* **Чтение истории**: Получение архивных (исторических) данных для узлов, которые поддерживают эту функцию.
* **Автодокументация**: Интерактивная документация API предоставляется автоматически через **Swagger UI** (`/docs`) и *
  *ReDoc** (`/redoc`).
* **Асинхронность**: Высокая производительность благодаря асинхронной работе с OPC UA сервером и клиентскими запросами.
...
| Клиент | <-------------> | FastAPI API | <-----------------> | OPC UA Сервер |
| (браузер, | JSON | Gateway | asyncua | (Kepware, |
| приложение)| | | | Ignition, |
+-------------+ +----------------+ +---------------+
```
